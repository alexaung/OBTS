...
namespace OBTS.API.Test
{
    class Program
    {
        ...
        static async Task RunAsync()
        {
            ...
            string apiBaseAddress = "http://localhost:57448/";

            CustomDelegatingHandler customDelegatingHandler = new CustomDelegatingHandler();

            HttpClient client = HttpClientFactory.Create(customDelegatingHandler);

            //var code = new Code { Id = Guid.NewGuid(), Type = "", Description = "", Value = "", Sequence = 1 };

            HttpResponseMessage response = await client.GetAsync(apiBaseAddress + "api/buses");//client.PostAsJsonAsync(apiBaseAddress + "api/code", code);
